...
#     flip_fit = mlrose.TSP(weights, values)
...
#     plot_montecarlo_sensitivity('TSP', 'sim_anneal', sa_results)
#     plot_montecarlo_sensitivity('TSP', 'sim_anneal_timing', sa_timing)
...
#     plot_montecarlo_sensitivity('TSP', 'rhc', rhc_results)
#     plot_montecarlo_sensitivity('TSP', 'rhc_timing', sa_timing)
...
#     plot_montecarlo_sensitivity('TSP', 'ga', ga_results)
#     plot_montecarlo_sensitivity('TSP', 'ga_timing', ga_timing)
...
#     plot_montecarlo_sensitivity('TSP', 'mimic', mimic_results)
#     plot_montecarlo_sensitivity('TSP', 'mimic_timing', mimic_timing)
#     all_results['MIMIC'] = [mimic_results, mimic_timing]


#     with open('./output/TSP/flipflip_data.pickle', 'wb') as handle:
#         pickle.dump(all_results, handle, protocol=pickle.HIGHEST_PROTOCOL)

    problem_size_space = np.linspace(5, 50, 2, dtype=int)
    ...
    for prob_size in problem_size_space:
        ...
        flip_fit = mlrose.TravellingSales.autogenerate(prob_size_int)
        ...
        problem = mlrose.TSPOpt(prob_size_int, maximize=False, coords=init_state)

        start = datetime.now()
        _, best_fitness_sa, fit_array_sa = mlrose.simulated_annealing(problem,
                                                                   schedule=mlrose.ExpDecay(exp_const=.0001, init_temp=5.),
                                                                   max_attempts=50, max_iters=3000,
                                                                   init_state=init_state, track_fits=True)
        ...
        start = datetime.now()
        _, best_fitness_rhc, fit_array_rhc = mlrose.random_hill_climb(problem, max_attempts=20, max_iters=3000,
                                                                  restarts=25, track_fits=True, init_state=init_state)
        ...
        start = datetime.now()
        _, best_fitness_ga, fit_array_ga =  mlrose.genetic_alg(problem, pop_size=prob_size_int*15, max_iters=10000,
                                                            mutation_prob=.15, max_attempts=200, track_fits=True)
        ...
        start = datetime.now()
        _, best_fitness_mimic, fit_array_mimic = mlrose.mimic(problem, pop_size=prob_size_int*10, max_iters=10000,
                                                        keep_pct=.1, max_attempts=100, track_fits=True)
    ...
    fit_frame.to_csv('./output/TSP/problem_size_fit.csv')
    time_frame.to_csv('./output/TSP/problem_size_time.csv')
    fit_iteration_frame.to_csv('./output/TSP/fit_per_iteration.csv')